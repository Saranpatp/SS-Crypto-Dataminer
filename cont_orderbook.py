...
async def main():
    exchange_ids = ['bittrex']
    exchanges = [getattr(ccxt.pro, exchange_id)() for exchange_id in exchange_ids]
    try:
        # Convert coroutines to tasks explicitly before passing to asyncio.wait
        tasks = {asyncio.create_task(watch_book(exchange, 'CELO/USD')) for exchange in exchanges}
